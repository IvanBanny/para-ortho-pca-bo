...
class Run:
    ...
    evals: int
    best: Solution
    extra_data: InitVar[dict] = None

    __lookup__ = {}
    __current_id__ = 1

    def __post_init__(self, extra_data):
        if extra_data:
        ...
        return Scenario(
            ...
            [
                Run(
                    ...
                    extra_data={k: v for k, v in run.items() if k not in ["instance", "evals", "best"]}
                )
...
class Dataset:
    ...
    def overview(self) -> pl.DataFrame:
        ...
        default_run_fields = {"data_id", "id", "instance", "evals", "best"}
        extra_run_fields = set()
        records = []

        for scen in self.scenarios:
            for run in scen.runs:
                extra_run_data = {k: v for k, v in vars(run).items() if k not in default_run_fields}
                extra_run_fields.update(extra_run_data.keys())

                records.append({
                    "base_data": [run.data_id] + meta_data + [scen.dimension, run.instance, run.id, run.evals,
                                                              run.best.y] + exattr_values,
                    "extra_data": extra_run_data
                })

        # Build schema and final records
        extra_fields = sorted(extra_run_fields)
        complete_schema = METADATA_SCHEMA + [(field, pl.String) for field in extra_fields] + exattr_schema

        final_records = [
            record["base_data"][:len(METADATA_SCHEMA)] +
            [record["extra_data"].get(field) for field in extra_fields] +
            record["base_data"][len(METADATA_SCHEMA):]
            for record in records
        ]

        return pl.DataFrame(final_records, schema=complete_schema, orient="row")
