...
def generate_random_array(
    lower_bounds: Union[float, List[float], np.ndarray],
...
def hill_valley_test(
    x_0: Union[np.ndarray, List[float]],
    fit_x_0: float,
    ...
    if isinstance(x_0, list):
        # Convert
        x_0 = np.array(x_0, dtype=float, copy=True, subok=False).ravel()
    
    if isinstance(x_1, list):
        # Convert
        x_1 = np.array(x_1, dtype=float, copy=True, subok=False).ravel()

    # Compute a difference vector
    diff_vect = np.subtract(x_1, x_0, dtype=np.float64)
    ...
    for k in r_points:
        ...
        # Compute x_test
        x_test = np.add(x_0, proportion * diff_vect, dtype=np.float64)
