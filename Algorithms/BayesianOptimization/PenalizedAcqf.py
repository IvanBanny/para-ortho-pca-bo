...
class PenalizedAcqf(AnalyticAcquisitionFunction):
    ...
    def _compute_penalty(self, X: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        # Compute distance to boundary for points outside the original bounds and the trust region
        distance_to_boundary = torch.sum(outside_lower + outside_upper + outside_trust_lower + outside_trust_upper,
                                             dim=-1)
