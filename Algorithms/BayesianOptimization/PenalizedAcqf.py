...
class PenalizedAcqf(AnalyticAcquisitionFunction):
    ...
    def __init__(
        ...
        Args:
            ...
            p_factor: Factor to control the strength of the penalty (default: 1e-3)
    ...
    def forward(self, X: Tensor) -> Tensor:
        ...
        if self.use_cont_log:
            ...
            return torch.log(self.acquisition_function(X) * penalty + self.epsilon)

        return torch.where(sum_q_distances == 0.0, self.acquisition_function(X), -sum_q_distances / self.p_factor)
